...
func (r *Rabbit) QueueBind(queueName, exchange, routingKey string) error {
	for {
...
// a demo function to handle consumer message
func handleConsumerMsg(msg amqp.Delivery) error {
	...
	trace := otel.Tracer("mq-trace-consumer")
	spanName := "consumer-span-name"
	_, span := trace.Start(ctx, spanName)
	span.SetStatus(codes.Ok, "consumer msg success")
	span.End()
	return nil
}
...
func (r *Rabbit) Producer(exchangeName, routingKey string, msg interface{}) error {
	for i := 0; i < 30; i++ {
		...
		if err := r.channel.Publish(
			...
			false,        // mandatory
			false,        // immediate
			generateMsg(amqp.Persistent, body),
		); err != nil {
...
func generateMsg(mode uint8, body []byte) amqp.Publishing {
	prop := otel.GetTextMapPropagator()
	parentCtx := context.Background()
	msg := amqp.Publishing{
	...
	ctx := prop.Extract(parentCtx, &MessagePCarrier{
		msg: amqp.Publishing{},
	})
	trace := otel.Tracer("mq-trace-producer")
	spanName := "producer-span-name"
	sonCtx, span := trace.Start(ctx, spanName)
	defer span.End()
	// send msg to kafka
	prop.Inject(sonCtx, &MessagePCarrier{msg: msg})
	span.SetStatus(codes.Ok, "producer msg success")
	return msg
}
